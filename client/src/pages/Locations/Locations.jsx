          ...
          <div className='LocationsSearchBarContainer'>
            ...
            {searchTyped ? (
              filteredAttics.map((attic) => {
                return <p className='SearchResult bg-white cursor-pointer p-2 shadow hover:bg-gray-200 transition duration-200 ease-in-out' onClick={() => { setSelectedAttic(attic); fetchAtticReviews() }}>{attic.location}</p>
              })
            ) : null}
          </div>
        </div>)}
